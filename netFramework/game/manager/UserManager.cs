    ...
    class UserManager
    {
        ...
        public static bool RegistrarUsuario(string nombre, string contraseña, int avatar, string colores, int edad, string email, string ip_actual)
        {
            ...
            try
            {
                ...
                client.SetParameter("contra", contraseña = Gcrypt(contraseña));
                client.SetParameter("avatar", avatar);
                client.SetParameter("colores", colores);
        ...
        public static UserInstance ObtenerUsuario(string nombre, string contraseña)
        {
            ...
            {
                client.SetParameter("nombre", nombre);
                client.SetParameter("password", contraseña);
                DataRow usuario2 = client.ExecuteQueryRow("SELECT * FROM usuarios WHERE nombre = @nombre AND password = @password LIMIT 1");
                if (usuario2 != null)
                {
                    UserInstance user = new UserInstance(usuario2);
                    return user;
                }
                else
                {
                    mysql client2 = new mysql();
                    client2.SetParameter("nombre", nombre);
                    client2.SetParameter("token_uid", contraseña);
                    DataRow usuario3 = client2.ExecuteQueryRow("SELECT * FROM usuarios WHERE nombre = @nombre AND token_uid = @token_uid LIMIT 1");
                    if (usuario3 != null)
                    {
                        UserInstance user = new UserInstance(usuario3);
                        return user;
                    }
                }
            }
